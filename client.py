...
# ... (Utility and Helper functions are unchanged) ...
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if is_gi_target:
            ...
            # --- ROBUST BATCH SELECTION LOGIC ---
            if attack_type in ["gradinversion", "gradinversion_plus"]:
                # For batch attacks, use the main dataloader
                batch_data, batch_target = next(iter(self.train_loader))
            else:
                # For single-image attacks, create a temporary loader with batch_size=1
        ...
        is_mp_malicious = (mp_params.get("enable", False) and self.client_id_numeric in mp_params.get("malicious_clients", []))
        if is_mp_malicious:
            attack_type = mp_params.get("type", "scaling")
            print(f"Client {self.client_id_numeric}: Applying model poisoning ({attack_type}).")
            if attack_type == "scaling": updated_params = scaling_attack(updated_params, mp_params.get("scale_factor", -1.0))
            elif attack_type == "sign_flip": updated_params = sign_flip(updated_params)
            elif attack_type == "noise": updated_params = add_noise(updated_params, mp_params.get("noise_scale", 0.5))
