...
def test(model: torch.nn.Module, test_loader: DataLoader, is_image: bool) -> Tuple[float, float]:
    ...
    with torch.no_grad():
        for data, labels in test_loader:
            ...
            # --- THE FIX: Reshape labels for binary classification loss ---
            if is_image:
...
# --- 1. Client Utility and Verification Functions ---
...
def calculate_verification_accuracy(
    ...
    """Calculates the absolute accuracy of a client's update on a verification set."""
    model = model_class(**model_args)
    set_parameters(model, client_update_params)
    _, accuracy = test(model, verification_loader, is_image)
    return accuracy

# --- 2. Data Leakage & Encryption (unchanged) ---
def my_func_image_leakage(original_image, reconstructed_image, alpha, beta, gamma) -> float:
    perceptual_similarity = ssim(original_image.squeeze(), reconstructed_image.squeeze(), data_range=original_image.max() - original_image.min())
